...
is_started(ProviderSpec,HostSpec)->
  %  io:format("ProviderSpec ,ProviderSpec  ~p~n",[{ProviderSpec ,HostSpec,?MODULE,?FUNCTION_NAME,?LINE}]),
    ...
    Result=case {ProviderExists,HostSpecExists} of
	       {false,_}->
		   io:format("Error ProviderExists,HostSpecExists ~p~n",[{ProviderExists,HostSpecExists,?MODULE,?FUNCTION_NAME,?LINE}]),
		   {error,["eexists ProviderExists,HostSpecExists",{ProviderExists,HostSpecExists},{HostSpecExists,HostSpec}]};
	       {_,false}->
		   io:format("Error ProviderExists,HostSpecExists ~p~n",[{ProviderExists,HostSpecExists,?MODULE,?FUNCTION_NAME,?LINE}]),
		   {error,["eexists ",{ProviderExists,ProviderSpec},{HostSpecExists,HostSpec}]};
	       {true,true}->
